  ...
  static fetchAll(cookieUser, cookieYear, cookieMonth) {
    ...
    if (cookieYear === undefined) {
      const balancesheetQuery = db.execute(
        `SELECT * FROM transactions WHERE '${cookieUser}' = user_id AND '${defaultMonth}' = month AND '${defaultYear}' = year AND "balance_sheet" = trans_type ORDER BY date DESC`
      );
      return balancesheetQuery;
    } else {
      console.log("Year/Month cookies recognized");
      const balancesheetQuery = db.execute(
        `SELECT * FROM transactions WHERE '${cookieUser}' = user_id AND '${cookieMonth}' = month AND '${cookieYear}' = year AND "balance_sheet" = trans_type ORDER BY date DESC`
      );
      return balancesheetQuery;
    }
  }
